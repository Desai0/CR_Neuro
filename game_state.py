...
class Tower:
    ...
    hp_box: Optional[Tuple[int, int, int, int]] = None # Координаты области, где мы искали HP
...
class GameState:
    ...
    my_broken_towers: List[Unit] = field(default_factory=list) # Используем Unit как заглушку, т.к. здоровья нет
    enemy_broken_towers: List[Unit] = field(default_factory=list)
    ...
    def __str__(self):
        ...
        return (
            ...
            f"My Broken: {len(self.my_broken_towers)}\n"
            f"Enemy Broken: {len(self.enemy_broken_towers)}\n"
